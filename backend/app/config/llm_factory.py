...
def get_llm():
    ...
    if provider == "huggingface":
        hf_api_key = os.getenv("HUGGINGFACEHUB_API_TOKEN") or os.getenv("HUGGINGFACE_API_KEY") or os.getenv("HF_TOKEN")
        hf_model = os.getenv("HUGGINGFACE_MODEL", "meta-llama/Llama-3.2-3B-Instruct")
        if not hf_api_key:
            raise ValueError("HF_TOKEN, HUGGINGFACEHUB_API_TOKEN or HUGGINGFACE_API_KEY is not set.")
